...
sayHelloButton.addEventListener('mouseover', changeColors);
...
const changeColorsBack = (event) => {
    event.target.style.backgroundColor = "#EFEFEF";
    event.target.style.color = "black";
}

sayHelloButton.addEventListener('mouseout', changeColorsBack);
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/${str}`).then((res) => {
...
const queryTest = (event) => {
    axios.get('http://localhost:3000/query-test/?age=54').then((res) => {
        console.log(res.data);
    })
}
